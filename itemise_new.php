...
class itemise extends FPDF {
    ...
    function itemise($master_details, $data, $summary, $grandtotal, $output_method = 'I') {
        //print_r($data);
        $util = new helper();
        $this->report_log = '['.$master_details[MASTER_MSISDN].']['.$master_details[ACCOUNT_NAME];
        $dir = "C:/xampp/htdocs/IcontrolBilling/bills/OCT2017/";
        $util->op_log($this->date,$this->report_log , $logdetail = 'GENERATING ITEMISED BILL' );
        $pdf = new PDF();
        if (count($master_details) == 0 & count($vas == 0 & count($voice) == 0 && count($sms) == 0)) {
            $util->op_log($this->date,$this->report_log, $logdetail = 'ERRRRRR ..NO DATA HAS BEEN PASSED' );
        ...
        //print_r($data);
        $i = 0;
        $file_name = $dir . str_replace(' ', '_', strtoupper($master_details[ACCOUNT_NAME])) . "_" . strtoupper($master_details[MASTER_MSISDN]).".pdf";
        foreach ($data as $msisdn => $elements) {
            $this->report_log = '['.$master_details[MASTER_MSISDN].']['.$master_details[ACCOUNT_NAME].']['.$msisdn.']';
            $util->op_log($this->date,$this->report_log, $logdetail = 'GENERATING DATA' );
